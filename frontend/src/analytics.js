...
export default function Analytics(props) {
  ...
  useEffect(() => {
    ...
    // get linechart data.
    fetch(
      "https://lchsuan.life:3001/api/analytics/linechart/" +
        Date_before_14.toISOString().slice(5, 10),
    )
      .then((res) => res.json())
      .then((data) => {
    ...
    fetch(
      "https://lchsuan.life:3001/api/analytics/linechart/" +
        Date_before_7.toISOString().slice(5, 10),
    )
      .then((res) => res.json())
      .then((data) => {
        let sum = 0;
    ...
    // Get barchart data.
    fetch(
      "https://lchsuan.life:3001/api/analytics/distribution/" +
        Date_before_7.toISOString().slice(5, 10),
    )
      .then((res) => res.json())
      .then((data) => {
    ...
    // Get yesterday data:
    fetch(
      "https://lchsuan.life:3001/api/analytics/yesterday/" +
        Date_before_2.toISOString().slice(5, 10),
    )
      .then((res) => res.json())
      .then((data) => {
        ...
        setyesterday(
          (
            ((data.data[1].total - data.data[0].total) * 100) /
            data.data[0].total
          ).toFixed(2),
        );
      });
      ...
      >
        ...
        >
          <StatBox
            ...
            subtitle={
              "Activities " +
              (total_14 - total_7 > total_7 ? "down by" : "up by")
            }
            progress={Math.abs(1 - (total_14 - total_7) / total_7).toString()}
            increase={
              total_14 - total_7 > total_7
                ? ((total_14 - total_7) / total_7).toFixed(2) * 100 - 100 + "%↓"
        ...
        >
          <StatBox
            ...
            increase={
              Math.abs(yesterday).toFixed(1) + (yesterday >= 0 ? "%↑" : "%↓")
            }
            icon={<TimelineIcon />}
          />
        </Box>
        ...
        >
          <StatBox
            ...
            increase={
              Math.abs((mag5 / total_14).toFixed(2) * 100).toString() + "%"
            }
            icon={<Looks5Icon />}
          />
        </Box>
